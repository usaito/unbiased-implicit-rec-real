...
def load_dataset(threshold: int = 4) -> Tuple:
    ...
    return train, val, test, pscore, num_users, num_items


def generate_real_data(threshold: int = 4) -> None:
    """Generate real-world data."""
    np.random.seed(12345)
    train, val, test, pscore, num_users, num_items = \
        load_dataset(threshold=threshold)
    ...
    np.save(str(path / 'pscore.npy'), arr=pscore.astype(np.int))
